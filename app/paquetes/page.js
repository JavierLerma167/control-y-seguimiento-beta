...
export default function Paquetes() {
  ...
  // --- FUNCIÓN PARA INICIALIZAR PAQUETES EN FIREBASE ---
    ...
    const unsubscribe = suscribir(COLLECTIONS.PAQUETES, async (data) => {
      if (isMounted.current) {
        if (data && data.length > 0) {
          ...
          // No hay paquetes, inicializar con datos por defecto
  ...
  // --- GUARDAR CAMBIOS EN FIREBASE (OPTIMIZADO) ---
    ...
    try {
      ...
      for (const pkg of nuevaLista) {
        if (existingMap.has(pkg.id)) {
          // Actualizar existente (solo ciertos campos)
      ...
      for (const [id] of existingMap) {
        // También eliminar imagen del Storage si existe
        const pkgAEliminar = paquetes.find(p => p.id === id);
  ...
  }, [usuario, esAdmin, guardando, leerTodos, actualizar, crear, eliminar, paquetes]);
  ...
  const handleImageUpload = async (id, file) => {
    ...
    try {
      // Obtener el paquete actual para eliminar imagen anterior
      ...
      // Actualizar paquete con la URL de la imagen
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 p-3 sm:p-4 md:p-8 font-light">
      ...
      {/* NAVBAR */}
      <nav className="sticky top-2 sm:top-4 z-50 bg-white border border-gray-200 p-2 sm:p-3 flex flex-wrap justify-between items-center mb-8 sm:mb-12 gap-2">
      ...
      {paso === 4 && esAdmin && (
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12 animate-in fade-in duration-300">
          <div className="border-b border-gray-200 pb-4 sm:pb-6">
            <h2 className="text-xl sm:text-2xl font-light tracking-tight mb-2">Editor de catálogo</h2>
            <p className="text-xs sm:text-sm text-gray-500">Modifica precios, textos e imágenes</p>
          </div>
          ...
          {categorias.map(cat => (
            <div key={cat} className="space-y-3 sm:space-y-4">
              ...
              <div className="grid gap-3 sm:gap-4">
                {paquetes.filter(p => p.categoria === cat).map(pkg => (
                  <div key={pkg.id} className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-4 p-3 sm:p-5 border border-gray-200 bg-white">
                    ...
                    <div className="lg:col-span-2">
                      <div className="aspect-square border border-gray-200 bg-gray-50 overflow-hidden relative group max-w-[200px] mx-auto lg:mx-0">
                        {pkg.img && <img src={pkg.img} className="w-full h-full object-cover" />}
                    ...
                    <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                      ...
                      <div className="col-span-1 md:col-span-2">
                        ...
                        <div className="flex justify-between items-center text-[8px] sm:text-[9px] text-gray-400 border-t border-gray-100 pt-2">
                          <span>Total: {pkg.incluye.length} beneficios</span>
                      ...
                      <div className="col-span-1 md:col-span-2 mt-2 p-2 sm:p-3 bg-gray-50 border border-gray-100">
                        ...
                        {pkg.incluye.length > 0 ? (
                          ...
                          <p className="text-[9px] sm:text-[10px] text-gray-400 italic">Sin beneficios</p>
                        )}
                      </div>
                    </div>
