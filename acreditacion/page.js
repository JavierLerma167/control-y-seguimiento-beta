// app/acreditacion/page.js (VERSIÓN FINAL CON BOTÓN DE CERRAR SESIÓN)
"use client";
...
export default function AcreditacionClientesPage() {
  ...
  const verificarExistenciaCliente = useCallback(async (ticket) => {
  ...
  useEffect(() => {
    ...
    const procesarNuevoRegistro = async (nuevoRegistro) => {
      ...
      try {
        // Verificar si ya existe usando el ticket
        const existe = await verificarExistenciaCliente(nuevoRegistro.ticket);
        ...
        if (!existe && isMounted.current) {
          ...
          notificacionTimeoutRef.current = setTimeout(() => {
            if (isMounted.current) {
              setNotificacion({ visible: false, nombre: '' });
            }
          }, 5000);
          ...
          const infoPaquete = typeof nuevoRegistro.paquete === 'object' ? nuevoRegistro.paquete : {
            id: null,
            nombre: nuevoRegistro.paquete || nuevoRegistro.paqueteNombre || 'SERVICIO SELECCIONADO',
            precio: nuevoRegistro.monto || nuevoRegistro.paquetePrecio || 0,
            categoria: nuevoRegistro.categoria || nuevoRegistro.paqueteCategoria || 'GENERAL',
            incluye: nuevoRegistro.paqueteIncluye || ['Información no disponible']
          };
          ...
          if (nuevoRegistro.id) {
            try {
              await eliminar(COLLECTIONS.COLA_ACREDITACION, nuevoRegistro.id);
            } catch (e) {
              console.error('Error eliminando de cola:', e);
            }
          }
        }
      } catch (error) {
        console.error('Error procesando registro de cola:', error);
        // Notificar al administrador si hay error crítico
        if (esAdmin && isMounted.current) {
          console.warn('⚠️ Error en cola de acreditación:', error.message);
        }
      }
    };
    
    // Suscribirse a la cola de acreditación
    const unsubscribeCola = suscribir(COLLECTIONS.COLA_ACREDITACION, (data) => {
      if (data && data.length > 0 && isMounted.current) {
        data.forEach(procesarNuevoRegistro);
      }
    }, (error) => {
      console.error('Error en suscripción de cola:', error);
    });
    
    return () => unsubscribeCola();
  }, [usuario, crear, eliminar, verificarExistenciaCliente, esAdmin]);
  ...
  const eliminarAsistente = async (id) => {
    if (!confirm("⚠️ ¿Eliminar registro de este cliente? Esta acción no se puede deshacer.")) return;
  ...
  const asistentesFiltrados = useMemo(() => {
    ...
    return asistentes.filter(a => 
      ...
      a.paquete?.toLowerCase().includes(busquedaLower) ||
      a.ticket?.toLowerCase().includes(busquedaLower)
    );
  }, [asistentes, busqueda]);

  // --- COMPONENTE TICKET (CORREGIDO - manejo de fechas) ---
  const TicketCompra = ({ data }) => {
    // Obtener fecha para mostrar en el ticket
    const getFechaDisplay = () => {
      if (data.fechaHora) {
        return new Date(data.fechaHora).toLocaleString();
      }
      if (data.fechaRegistro) {
        return new Date(data.fechaRegistro).toLocaleString();
      }
      if (data.fecha) {
        return data.fecha;
      }
    ...
    // Obtener fecha para el QR (formato ISO)
    const getFechaQR = () => {
      if (data.fechaHora) return data.fechaHora;
      if (data.fechaRegistro) return data.fechaRegistro;
      ...
      <div className="bg-white border-2 border-gray-900 p-4 sm:p-6 font-mono text-xs sm:text-sm relative max-w-md mx-auto">
        ...
        <div className="space-y-0.5 sm:space-y-1 mb-3 sm:mb-4 text-[10px] sm:text-[11px]">
          <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
            ...
            <span className="font-medium break-words">{data.nombre || 'N/A'}</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
            <span className="text-gray-500">EMAIL:</span>
            <span className="break-words">{data.email || 'N/A'}</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
            ...
            <span className="text-right text-[9px] sm:text-[9px] break-words">{data.direccion || 'N/A'}</span>
          </div>
        </div>

        <div className="border-t border-dashed border-gray-300 pt-3 sm:pt-4 mb-3 sm:mb-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0 mb-2">
            <span className="text-xs sm:text-xs font-bold break-words">{data.paquete || data.paqueteNombre || 'SERVICIO'}</span>
            <span className="text-xs sm:text-sm font-bold">{data.nota || `$${data.paquetePrecio || 0}`}</span>
          </div>
          
          {(data.paqueteInfo?.incluye || data.paqueteIncluye) && (
            <div className="text-[8px] sm:text-[9px] text-gray-500 space-y-0.5 pl-2">
              {(data.paqueteInfo?.incluye || data.paqueteIncluye || []).map((item, idx) => (
          ...
          <div className="border border-gray-200 p-2 sm:p-3 bg-white">
            <QRCode 
              ...
              value={JSON.stringify({ 
                ticket: data.ticket,
                cliente: data.nombre,
                paquete: data.paquete || data.paqueteNombre,
                fecha: getFechaQR()
              })} 
            />
          </div>
        ...
        <div className="border-t-2 border-dashed border-gray-300 pt-3 sm:pt-4 mt-2">
          <div className="flex flex-col sm:flex-row sm:justify-between text-base sm:text-base font-bold mb-2 gap-1 sm:gap-0">
            <span>TOTAL</span>
            <span>{data.nota || `$${data.paquetePrecio || 0}`}</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="mt-4 text-sm text-gray-400">Cargando sistema de acreditaciones...</p>
            </div>
          </div>
        </div>
      </main>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-gray-200 pb-4 sm:pb-6 mb-6 sm:mb-10 gap-3 sm:gap-0">
          <div className="w-full md:w-auto">
            <nav className="mb-2 sm:mb-3 flex items-center justify-between gap-4">
          ...
          <section className="lg:col-span-4 space-y-6 sm:space-y-8">
            <div className="bg-gray-50 p-4 sm:p-6 border border-gray-200">
              ...
              <form onSubmit={registrarCliente} className="space-y-3 sm:space-y-4">
                ...
                >
                  {guardando ? '⏳ Registrando...' : '🎟️ Generar acreditación'}
                </button>
              </form>
            </div>
          ...
          <section className="lg:col-span-8">
            <div className="mb-4 sm:mb-6">
              <div className="relative">
                <input 
                  type="text" 
                  placeholder="🔍 Buscar por nombre, ticket o paquete..." 
                  className="w-full border border-gray-200 px-3 sm:px-4 py-2 sm:py-3 text-sm focus:border-gray-400 outline-none transition-colors pl-9"
                  value={busqueda}
                  onChange={(e) => setBusqueda(e.target.value)}
                />
                <span className="absolute left-3 top-2.5 sm:top-3.5 text-gray-400 text-sm">🔍</span>
              </div>
            </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              {asistentesFiltrados.length === 0 ? (
                <div className="py-12 sm:py-16 text-center border border-gray-100 text-sm text-gray-400">
                  {busqueda ? `🔍 No hay resultados para "${busqueda}"` : '📭 Sin registros'}
                </div>
              ) : (
                  ...
                  >
                    <div className="flex-1 min-w-[250px] w-full sm:w-auto mb-3 sm:mb-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className="text-[8px] sm:text-[9px] bg-gray-200 px-1.5 py-0.5 font-mono rounded">
                          #{a.ticket?.slice(-8) || 'N/A'}
                        </span>
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
                        <span className="font-medium break-words">📦 {a.paquete || a.paqueteNombre}</span>
                        <span className="text-gray-400">|</span>
                        <span className="text-gray-700">💰 {a.nota || `$${a.paquetePrecio || 0}`}</span>
                      </div>
                      
                      <div className="text-[9px] sm:text-[10px] text-gray-400 mt-1">
                        📅 {a.fechaHora ? new Date(a.fechaHora).toLocaleDateString() : a.fecha}
                      </div>
                    </div>
